...
contains
    ...
    subroutine init_satzoneGW(nac, &
        ...
        ! SS February 2021 changed this and these types are now in dyna_common.f90
        ...
        !     enddo
            ...
            !! ================================================================
            !! MPR check to make sure this initialisation will not crash the code
            !! We have crashes when exp(-sd/m2) = 0
            !! If it will crash, set deficits to 0 (all stores full)
            !first a check to see where we are close to 0:
    !         init_fullstores = .false.
    !         do ia = 1,nac
    !             if ((-dyna_hru(ia)%sd/szm(dyna_hru(ia)%ipar))<-700) then
    ! !                PRINT *,'   ia = ',ia,': -SD/SZM = ',&
    ! !                    (-dyna_hru(ia)%sd/szm(dyna_hru(ia)%ipar)), &
    ! !                    '... so the model may have crashed.'
    !                 init_fullstores = .true.
    !             end if
    !         end do
    
    !         !if problem areas were found, initialise to full.
    !         if (init_fullstores) then
    !             print *,'USING ALTERNATIVE INITIALISATION SCHEME',&
    !                 ' ...SETTING ALL DEFICITS TO 0.'
    !             sol_flag = 3
    !             call calculate_qbmax(dyna_hru, qb_tmp, &
    !                 nac,num_rivers,dt, rivers)
    !         end if
            ...
            ! ! else if (sol_flag.eq.3) then
            ! !     write(40, *) 'Incompatible sd and SZM values will cause model crashes - deficits set to 0 and max QBFs'
            ! end if
            ...
            !YZ GW change, comment out not needed variables 
            do ia = 1, nac
                ...
                !YZ ini srz storage
                dyna_hru(ia)%srzini = SRmax (dyna_hru(ia)%ipar)*0.5
                dyna_hru(ia)%srz = dyna_hru(ia)%srzini
                dyna_hru(ia)%qbfini= 0
                dyna_hru(ia)%qbf = dyna_hru(ia)%qbfini
            ...
            !
            call checked_allocate(s_full, size(pf_data%location), nstep)
            do i = 1, nstep
                do iriv = 1, size(pf_data%location)
                    q (iriv, i) = 0
                end do
            end do
    ...
    end module dyna_init_satzoneGW
