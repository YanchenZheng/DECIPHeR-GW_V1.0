...
contains
    ...
    ! **********************************************************************************************
    ! subroutine: set_nodata_value
    ! **********************************************************************************************
    ! This function loops through an .asc map replacing all occurances of one value with another
    ! intended for ensuring consistency between nodata values within the MPR code.

    subroutine set_nodata_value( map, old_nodata_value, new_nodata_value)
        ...
        double precision, intent(in)                       :: old_nodata_value
        double precision, intent(in)                       :: new_nodata_value
        ...
        nrows_map = size(map(:,1))
        ncols_map = size(map(1,:))

        !loop through map - replacing old values with new
        DO i=1,nrows_map
            DO j=1,ncols_map
                IF (map(i,j) == old_nodata_value) THEN
                    map(i,j) = new_nodata_value
                END IF
            END DO
        END DO


    end subroutine set_nodata_value
    ...
    ! **********************************************************************************************
    ! subroutine: calc_exp_fit
    ! **********************************************************************************************
    ! This function calculates an exponential fit using linear least squares regression.
    ! Exponential function takes the form: y = a * exp(b*x)
    ! input is a 2 column array - first column gives y values, second column x values.
    ! output - an extra column will be added to fit_data giving fit predictions.
    ! output - a and b are the fit constants.
    ! - Rosie Lane, 11/06/2018

    subroutine calc_exp_fit(fit_data, a, b)
        ...
        !declare dummy variables
        double precision, dimension(:,:), intent(inout)     :: fit_data
        double precision, intent(out)                       :: a
        double precision, intent(out)                       :: b

        !local variables
        complex, allocatable, dimension(:)         :: ln_y, x  !x, and y variables for regression
        integer          :: i
        integer          :: n_points
        double precision :: m_x, m_y                !mean of x and mean of y
        double precision :: SSx=0, SSy=0, SSxy=0    !sum of squared errors
        double precision :: slope, intercept        !slope and intercept of linear fit

        !allocate variables
        n_points  = SIZE(fit_data(:,1))
        allocate(ln_y(n_points))
        allocate(x(n_points))

        !take the logarithm of both sides to transform for linear regression
        DO i=1,n_points
            ln_y(i) = LOG(CMPLX(fit_data(i,1)))
        END DO
        x = fit_data(1:n_points,2)

        !calculate means:
        m_x = SUM(x) / n_points
        m_y = SUM(ln_y) / n_points
        ...
        SSxy=0
        DO i = 1,n_points
            SSx = SSx + ((x(i) - m_x)*(x(i) - m_x))
            SSy = SSy + ((ln_y(i) - m_y)*(ln_y(i) - m_y))
            SSxy = SSxy + ((x(i) - m_x) * (ln_y(i) - m_y))
        END DO
        ...
        !Return to exponential fit
        a = exp(intercept)
        b = slope

        !add calculated values to fit_data
        DO i = 1,n_points
            fit_data(i,3) = a * exp(b*x(i))
        END DO
...
!        print *, 'SSx, SSy, SSxy = ',SSx, SSy, SSxy
!        print *, 'slope = ',slope
...
! **********************************************************************************************
    ! The following subroutines are from dta_utility
    ! **********************************************************************************************
    ! Very useful functions for spatial analysis, importing and exporting .asc files and lists
	! useful utility functions for processing spatial grids
	! reading, writing and unit conversions
	!% Toby Dunne
	!% Apr 2016
...
98      format (F0.1,A,F0.1,A,I0,A,I0)
    ...
    subroutine write_numeric_list(filename, col_headers, data, data_precision)
        ...
        ! similar to write_numeric_list but file must already be open
    subroutine write_numeric_list_append(col_headers, data, data_precision, unit)
        implicit none
        ! Argument declares
        character(64) :: col_headers(:)
        integer :: data_precision
