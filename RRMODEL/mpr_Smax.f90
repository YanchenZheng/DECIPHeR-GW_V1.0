...
!! This module contains all subroutines involving MPR for the Smax parameter:
!
!   get_BasPred_Smax    Complete logical list of basin predictors required
!   init_gp_Smax        Initialise global parameters
!   pedotf_Smax         Pedo-transfer function
...
contains
    ...
    subroutine get_BasPred_Smax(pedo_tf_Smax, req_soils)

        implicit none

        ! declare dummy arguments
        integer, intent(inout)                     :: pedo_tf_Smax !pedo_tf_equation to be used
        logical, dimension(*), intent(inout)  :: req_soils    !soils data required logical

        ! define required BasinPredictors for LnTo
        select case(pedo_tf_Smax)
            ...
            case (2)     !ptf based on soil depth

                req_soils(14) = .true.   !soil depth to rock map
                req_soils(17) = .true.   !porosity map

            case (3)    !ptf based on soil depth - with addition for productive hydrogeology

                req_soils(14) = .true.   !soil depth to rock map
                req_soils(17) = .true.   !porosity map
                req_soils(18) = .true.   !productive hydrogeology map (1=high productive, 0=low/mod productive)

            case (4)
                print *, "ERROR: A second transfer function has not yet been programmed for Smax"
                print *, "Select pedo_tf = 0 to treat it as a fixed parameter"
                print *, "Select pedo_tf = 1 to treat it as a global parameter"
                 print *, "pedo_tf_Smax = 2, sets a ptf based on a soil depth map"
                stop

            case default
                print *, "WARNING: a valid pedo-tf equation for Smax must be specified in the control file"
                print *, "Smax will be set to the default of fixed parameter"
                print *, "The following options can be selected in the control file:"
                print *, "pedo_tf_Smax = 0, sets it as a fixed parameter"
                print *, "pedo_tf_Smax = 1, sets it as a global parameter"
                print *, "pedo_tf_Smax = 2, sets a ptf based on a soil depth map"
                !print *,
    ...
    end subroutine get_BasPred_Smax
    ...
    ! **********************************************************************************************
    ! subroutine: init_gp_Smax : Initialise global parameters for Smax
    ! **********************************************************************************************
    ...
    subroutine init_gp_Smax(pedo_tf_Smax,glob_pms_Smax, n_pm_maps, start_seed, n_gp_all,pm_range)
        ...
        integer, intent(in)                                         :: pedo_tf_Smax
        double precision, allocatable, dimension(:,:),intent(inout) :: glob_pms_Smax      !global param list
        ...
        select case(pedo_tf_Smax)
            ...
            case (0)    !Treat as fixed parameter (for ease this is a global parameter with a 0 range)
                ...
                print *, "Smax is a fixed parameter, of value ",pm_range(2)
                n_glob_pms = 1
            ...
            case (1)    !Treat as global parameter
                ...
                print *, "Smax is a global parameter, in the range ",pm_range(1)," ",pm_range(3)
                n_glob_pms = 1
                allocate(min_gp(1))         !min and max values have length 1, as we have 1 global parameter
                allocate(max_gp(1))
            ...
            case (2)    !Pedotransfer function!!!
                ...
                print *, "Smax will use a pedotransfer function based on soil depth"
                print *, "Smax = (a * soil_depth)"
            ...
            case (3)    !Pedotransfer function, with addition for high productivity geology

                print *, "Smax will use a pedotransfer function based on soil depth and hydrogeology"
                print *, "Smax = (a * soil_depth * porosity) + (b)"
            ...
            case default
                print *, "ERROR: pedo_tf_Smax incorrectly defined. "

        end select

        !2. From above, set sizes of the global parameter array and number of random numbers to produce.
        n_gp_all(7) = n_glob_pms

        allocate(glob_pms_Smax(n_glob_pms, n_pm_maps))
        ...
        DO i = 1,n_glob_pms
            DO j = start_seed, start_seed+n_pm_maps-1
                ...
                glob_pms_Smax(i, j-start_seed+1) = num

            END DO
            IF (n_pm_maps >= 2) THEN
                print *, 'First 2 values for parameter ',i,' : ',glob_pms_Smax(i,1:2)
            END IF
        END DO
    ...
    ! **********************************************************************************************
    ! subroutine: pedotf Smax: Pedo-transfer function for Smax
    ! **********************************************************************************************
    ...
    subroutine pedotf_Smax(n_i, pedo_tf, glob_pms,bp,porosity,hydrogeo,pm_map)
        ...
        real, dimension(:,:), allocatable, intent(in)                :: bp          !basin predictor - soil depth map
        real, dimension(:,:), intent(in)                             :: porosity    !basin predictor - porosity map
        real, dimension(:,:), intent(in)                             :: hydrogeo    !basin predictor - is high productive hydrogeology?
        ...
        select case(pedo_tf)
            ...
            case (2)    !Pedotransfer function!!! - Smax = (a * soil_depth * porosity)

                n_glob_pms = 1
                DO i = 1, nrows_bp
                    DO j = 1, ncols_bp

                        pm_map(i,j) = glob_pms(1,n_i) * bp(i,j) * porosity(i,j)
                        !pm_map(i,j) = pm_map(i,j) + glob_pms(2,n_i)

                        !deal with -9999 values
                        IF((bp(i,j)==-9999).OR.(porosity(i,j)==-9999)) THEN
                            pm_map(i,j) = 0.5 !old default value
                        ENDIF

                    END DO
                END DO

            case (3)    !Pedotransfer function!!! - Smax = (a * soil_depth * porosity) + (b * high_prod_geology?)
                ...
                DO i = 1, nrows_bp
                    DO j = 1, ncols_bp
                        ...
                        IF (hydrogeo(i,j) == 1) THEN !areas overlaying high productivity geology.
                            pm_map(i,j) = pm_map(i,j) + glob_pms(2,n_i)
                        END IF
         ...
         ! set a limit - to stop crazy values causing crashes!
        DO i = 1,nrows_bp
            DO j = 1,ncols_bp
                !add in upper and lower caps
                IF (pm_map(i,j)<0.001) THEN
                    pm_map(i,j)=0.001
