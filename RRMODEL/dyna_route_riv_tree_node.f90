module dyna_route_riv_tree_node
...
contains
    ...
    function get_flow(riv, tdh, node_index, full_route) result(q_sum)
        implicit none
        ...
        ! add flow for full river network - don't perform this step if reach only mode
    ...
    subroutine set_flow(riv, tdh, node_index, full_route, q_value)
        implicit none
        type(route_river_info_type) :: riv
        type(route_time_delay_hist_type) :: tdh
        integer :: node_index
        ...
        ! need to check which rivers to use from the node's upstream_tree
        ! the read index will be the same for each row
        outlet_index = tdh%node_hist_indexes(node_index,1)


        ! set the flow from this reach
        tdh%flow_matrix(node_index,outlet_index) = q_value
        ...
        if (full_route.eq.1) then
            ...
            do j=1,riv%node_list(node_index)%upstream_tree_count
                ...
                !zero all tributaries in flow_matrix row
                tdh%flow_matrix(upstream_index,outlet_index) = 0
    ...
    function get_node_index(node_list, node_count, gauge_id) result(node_index)
        implicit none
        integer :: node_count
        integer :: gauge_id
        type(riv_tree_node), dimension(node_count) :: node_list
        integer :: node_index
        ...
        do i = 1, node_count
            if(node_list(i)%gauge_id == gauge_id)then
                node_index = i
                exit
            endif
        end do
        ...
        if(node_index == -1) then
            print *, 'Gauge ID not found', gauge_id
            !stop
        end if
...
end module dyna_route_riv_tree_node
