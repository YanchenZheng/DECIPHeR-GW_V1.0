...
contains
    ...
    !         call route_process_run_step(riv, tdh, pf_data, &
    !             node_to_flow_mapping, &
    ...
    subroutine route_process_run_step(riv, tdh, pf_data, &
        node_to_flow_mapping, &
        step_flow_input, &
        step_flow_output, &
        s_full_step)
        ...
        double precision, intent(out) :: s_full_step(:)
        ! locals
        integer :: i, j
        ...
        if(pf_data%pf_enabled) then
            call pf_process(tdh, riv, pf_data)
        endif
        ...
        do i=1,size(riv%flow_output_indexes)
            ...
            step_flow_output(i) = get_flow(riv, tdh, node_index, full_route)
        end do
        ...
        if(pf_data%pf_enabled) then
            do i = 1, size(pf_data%reservoir_param)
                s_full_step(i) = pf_data%reservoir_param(i)%storage 
            end do
        endif
...
end module dyna_route_processing
