    ...
    subroutine dynamic_distGW (nac, &
        ...
        do ia = 1, nac
            ...
            do iriv = 1, num_rivers
                ...
                !dtt *
                if (dyna_hru(ia)%ipriv==iriv) then
                    dyna_riv(iriv)%qb = dyna_riv(iriv)%qb + dyna_hru(ia)%qbf * dyna_hru(ia)%ac
                    dyna_riv(iriv)%sumqb = dyna_riv(iriv)%sumqb + dyna_hru(ia)%qbf * dyna_hru(ia)%ac
                    !dyna_riv(iriv)%sumqb = dyna_riv(iriv)%sumqb + dyna_riv(iriv)%qb
                endif
                !              * rivers (ia, iriv)
                ! print*,rivers (ia, iriv)
...
end module dyna_dynamic_distGW
