...
!! This module contains all subroutines involving MPR for the CHV parameter:
!
!   get_BasPred_CHV    Complete logical list of basin predictors required for LnTo
!   init_gp_CHV        Initialise global parameters for LnTo
!   pedotf_CHV         Pedo-transfer function for LnTo
...
contains
    ...
    subroutine get_BasPred_CHV(pedo_tf_CHV, req_soils)

        implicit none

        ! declare dummy arguments
        integer, intent(inout)                     :: pedo_tf_CHV !pedo_tf_equation to be used
        logical, dimension(*), intent(inout)  :: req_soils    !soils data required logical

        ! define required BasinPredictors for LnTo
        select case(pedo_tf_CHV)
            ...
            case (2)     !Use a pedo-transfer equation
                print *, "ERROR: A transfer function has not yet been programmed for CHV"
                print *, "Select pedo_tf = 0 to treat it as a fixed parameter"
                print *, "Select pedo_tf = 1 to treat it as a global parameter"


            case default
                print *, "WARNING: a valid pedo-tf equation for CHV must be specified in the control file"
                print *, "CHV will be set to the default of fixed parameter"
                print *, "The following options can be selected in the control file:"
                print *, "pedo_tf_CHV = 0, sets it as a fixed parameter"
                print *, "pedo_tf_CHV = 1, sets it as a global parameter"
                !print *,
    ...
    end subroutine get_BasPred_CHV
    ...
    ! **********************************************************************************************
    ! subroutine: init_gp_CHV : Initialise global parameters for CHV
    ! **********************************************************************************************
    ...
    subroutine init_gp_CHV(pedo_tf_CHV,glob_pms_CHV, n_pm_maps, start_seed, n_gp_all,pm_range)
        ...
        integer, intent(in)                                         :: pedo_tf_CHV
        double precision, allocatable, dimension(:,:),intent(inout) :: glob_pms_CHV      !global param list
        integer, intent(in)                                         :: n_pm_maps
        integer, intent(in)                                         :: start_seed   !to start rand num generator
        integer, dimension(9), intent(inout)                        :: n_gp_all     !vector of number of global params for all params
        ...
        select case(pedo_tf_CHV)
            ...
            case (0)    !Treat as fixed parameter (for ease this is a global parameter with a 0 range)
                ...
                print *, "CHV is a fixed parameter, of value ",pm_range(2)
                n_glob_pms = 1
            ...
            case (1)    !Treat as global parameter
                ...
                print *, "CHV is a global parameter, in the range ",pm_range(1)," ",pm_range(3)
                n_glob_pms = 1
                allocate(min_gp(1))         !min and max values have length 1, as we have 1 global parameter
                allocate(max_gp(1))
            ...
            case default
                print *, "ERROR: pedo_tf_CHV incorrectly defined. "

        end select

        !2. From above, set sizes of the global parameter array and number of random numbers to produce.
        n_gp_all(5) = n_glob_pms

        allocate(glob_pms_CHV(n_glob_pms, n_pm_maps))
        ...
        DO i = 1,n_glob_pms
            DO j = start_seed, start_seed+n_pm_maps-1
                ...
                glob_pms_CHV(i, j-start_seed+1) = num

            END DO
            IF (n_pm_maps >= 2) THEN
                print *, 'First 2 values for parameter ',i,' : ',glob_pms_CHV(i,1:2)
            END IF
        END DO
    ...
    subroutine pedotf_CHV(n_i, pedo_tf, glob_pms,bp, pm_map)
