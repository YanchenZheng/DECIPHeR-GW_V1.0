...
contains
    ...
    subroutine project (auto_start_file, &
        out_dir_path, &
	    filename_mpr_control, &
	    filename_mpr_filemgr)
        ...
        character(1024), intent(out) :: filename_mpr_control, filename_mpr_filemgr
        ...
        character(900), dimension(:), allocatable :: filename_mpr_control_all
	    character(900), dimension(:), allocatable :: filename_mpr_filemgr_all
        ...
        if (allocatestatus /= 0) stop "*** Not enough memory ***"
        allocate(filename_mpr_control_all(num_proj))
        if (allocatestatus /= 0) stop "*** Not enough memory ***"
        allocate(filename_mpr_filemgr_all(num_proj))
        ...
        do i = 1, num_proj
            ...
            read(10,525) comment, filename_mpr_control_all(i)
            read(10,525) comment, filename_mpr_filemgr_all(i)
    ...
    filename_mpr_control = trim(filename_mpr_control_all(id_project))
    filename_mpr_filemgr = trim(filename_mpr_filemgr_all(id_project))
