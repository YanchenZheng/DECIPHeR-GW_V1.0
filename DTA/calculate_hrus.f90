program calculate_hrus
    ...
    type(hru_class_struct) :: hru_class(9)
    ...
    do i = 1, size(gauges_all, 1)
        ...
        call reclass_grid(int(catch_data), int(nodata), hru_class(9)%class_array,  hru_class(9)%meta)

        allocate(hru_class_meta(size(hru_class(9)%meta, 1), 2))
        allocate(hru_class_array(size(hru_class(9)%class_array, 1), size(hru_class(9)%class_array, 2)))
        hru_class_meta(:, 1) = hru_class(9)%meta(:, 1)
        hru_class_meta(:, 2) = hru_class(9)%meta(:, 1)

        hru_class_array = hru_class(9)%class_array
        ...
        allocate(rivs_meta(size(hru_class(9)%meta, 1), size(hru_class(9)%meta, 2)))

        rivs = 0
        do k = 1, size(hru_class(9)%meta, 1)
            where (int(catch_data).eq.hru_class(9)%meta(k, 2))
                rivs = hru_class(9)%meta(k,1)
            end where
        end do
        rivs_meta = hru_class(9)%meta
        deallocate(catch_data)
        ...
        do k = 1, 8
            ...
            if (len_trim(hru_class(k)%fn_end).gt.0) then
                ...
                if (hru_class(k)%class_type(1:4).eq.'TOPO') then
                    ...
                    call calc_topo_class(hru_class, k, nodata, fn_log)
